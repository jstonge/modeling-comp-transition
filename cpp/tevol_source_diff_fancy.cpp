 ...
 */
 ...
 int main(int argc, const char *argv[]) {
     ...
     double ktau = stod(argv[4]);
     double a = stod(argv[5]);
     double bl = stod(argv[6]);
     double kc = stod(argv[7]);
     double x0c = stod(argv[8]);
     double K = stod(argv[9]);
     int max1 = stoi(argv[10]);
     int max2 = stoi(argv[11]);
     int IC = stoi(argv[12]);
     int TEMPORAL = atoi(argv[13]);

     double t_max = (argc > 14) ? stod(argv[14]) : 10.0;

     Sparam param = {mu, nu_n, nu_p, ktau, a, bl, kc, x0c, K, max1, max2};
